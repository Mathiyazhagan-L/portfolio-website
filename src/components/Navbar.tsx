...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/98 dark:bg-slate-900/98 backdrop-blur-2xl border-b border-slate-300 dark:border-slate-800 px-4 pt-3 pb-6 space-y-2 shadow-xl">
          {navItems.map((item) => {
            const Icon = item.icon;
            return (
              <a
                ...
                className="flex items-center gap-3 px-4 py-2.5 rounded-2xl text-sm font-bold text-[#10103B] dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
              >
                <Icon className="w-4 h-4 text-[#10103B] dark:text-slate-300" />
                <span className="font-bold">{item.label}</span>
              </a>
            );
          })}
